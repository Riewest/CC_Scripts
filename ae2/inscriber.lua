...
function moveItem(fromInv, fromSlot, toInv, toSlot )
    --print(tostring(fromSlot)..":"..fromInv..">"..tostring(toSlot)..":"..toInv)
...
function processOutput()
    if inscribers[COMBINATION] then
        local combo = peripheral.wrap(inscribers[COMBINATION].name)
        if combo.getItemDetail(SLOTS.output) then
...
function startup()
    ...
    --print("Combo:",inscribers[COMBINATION].name)
    --print("outInv:",output_inv)
end
