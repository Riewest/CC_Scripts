...
function process_slot(to_inv, from_inv)
    to_inv.pullFluid(peripheral.getName(from_inv), from_slot)
end
...
function process_invs()
    if peripheral.isPresent(to_inv_name) and peripheral.isPresent(from_inv_name) then
        ...
        local tanks = from_inv.tanks()

        for k,v in pairs(tanks) do
            from_inv.pushFluid(to_inv_name, v.amount)
        end        
    end
end
